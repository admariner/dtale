...
export const DataViewer: React.FC = () => {
  ...
  const getData = (updatedIds: number[], refresh = false): void => {
    ...
    DataRepository.load(dataId, params).then((response) => {
      ...
      if (response) {
        const formattedData = response.results.reduce(
          (res, r) => ({
            ...res,
            [r[gu.IDX]]: Object.keys(r).reduce(
              (res2, col) => ({
                ...res2,
                [col]: gu.buildDataProps(response.columns.find(({ name }) => name === col)!, r[col], settings),
              }),
              {},
            ),
          }),
        ...
        setColumns(
          updatedColumns.map((c) => ({
            ...c,
            ...gu.calcColWidth(c, data, updatedRowCount, settings.sortInfo, settings.backgroundMode, maxColumnWidth),
          })),
        );
  ...
  React.useEffect(() => {
    const refresh = JSON.stringify(settings) !== JSON.stringify(previousSettings);
  ...
  const propagateState = (state: Partial<PropagatedState>, callback?: () => void): void => {
    ...
    if (state.hasOwnProperty('renameUpdate')) {
      finalState.data = state.renameUpdate?.(data) ?? data;
    }
