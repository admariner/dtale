...
describe('DataViewer tests', () => {
  ...
  it('DataViewer: column range selection', async () => {
    ...
    expect(result.find(DataViewer).instance().state.columnRange).toEqual({
      start: 1,
      end: 1,
    });
    ...
    expect(buildCopyColumnsSpy).toBeCalledWith('1', { columns: `["col1","col2"]` });
  ...
  it('DataViewer: column ctrl selection', async () => {
    ...
    result = result.update();
    expect(result.find(DataViewer).instance().state.ctrlCols).toEqual([1]);
    instance = result.find(Header).at(2);
    await act(async () => {
      instance.find('div.headerCell').find('.text-nowrap').simulate('click', { ctrlKey: true });
    });
    result = result.update();
    expect(result.find(DataViewer).instance().state.ctrlCols).toEqual([1, 2]);
    instance = result.find(Header).at(1);
    await act(async () => {
      instance.find('div.headerCell').find('.text-nowrap').simulate('click', { ctrlKey: true });
    });
    result = result.update();
    expect(result.find(DataViewer).instance().state.ctrlCols).toEqual([2]);
    result.unmount();
  });
});
